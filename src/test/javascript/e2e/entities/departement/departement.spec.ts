...
describe('Departement e2e test', () => {
  ...
  it('should load Departements', async () => {
    ...
    expect(await departementComponentsPage.getTitle()).to.eq('hospitalApp.departement.home.title');
    await browser.wait(
  ...
  it('should load create Departement page', async () => {
    ...
    expect(await departementUpdatePage.getPageTitle()).to.eq('hospitalApp.departement.home.createOrEditLabel');
  ...
  it('should create and save Departements', async () => {
    ...
    await promise.all([departementUpdatePage.setLibelleInput('libelle')]);

    expect(await departementUpdatePage.getLibelleInput()).to.eq('libelle', 'Expected Libelle value to be equals to libelle');
  ...
  it('should delete last Departement', async () => {
    ...
    expect(await departementDeleteDialog.getDialogTitle()).to.eq('hospitalApp.departement.delete.question');
